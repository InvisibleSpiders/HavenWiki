# Permission Index

## Teleport

- `havennecessities.teleport.spawn`
- `havennecessities.teleport.setspawn`
- `havennecessities.teleport.home`
- `havennecessities.teleport.sethome`
- `havennecessities.teleport.delhome`
- `havennecessities.teleport.homes`
- `havennecessities.teleport.warp`
- `havennecessities.teleport.setwarp`
- `havennecessities.teleport.delwarp`
- `havennecessities.teleport.warps`
- `havennecessities.teleport.playerwarp`
- `havennecessities.teleport.setplayerwarp`
- `havennecessities.teleport.delplayerwarp`
- `havennecessities.teleport.pwarps`
- `havennecessities.teleport.tpa`
- `havennecessities.teleport.tpahere`
- `havennecessities.teleport.tpaccept`
- `havennecessities.teleport.tpdeny`
- `havennecessities.teleport.rtp`
- `havennecessities.teleport.tppos`
- `havennecessities.economy.bypass.teleport-costs`

## Economy And Banks

- `havennecessities.balance.view`
- `havennecessities.balance.view.others`
- `havennecessities.balance.pay`
- `havennecessities.bank.use`
- `havennecessities.bank.deposit`
- `havennecessities.bank.withdraw`
- `havennecessities.bank.upgrade`
- `havennecessities.serverbank.view`
- `havennecessities.serverbank.deposit`

## Player Systems

- `havennecessities.flight.use`
- `havennecessities.flight.status`
- `havennecessities.flight.upgrade`
- `havennecessities.storage.use`
- `havennecessities.afk.use`
- `havennecessities.chat.use`
- `havennecessities.chat.item-link`
- `havennecessities.chat.mention`
- `havennecessities.chat.channel.market`
- `havennecessities.chat.channel.staff`
- `havennecessities.chat.cooldown.bypass.market`
- `havennecessities.chat.discord.link`
- `havennecessities.custommessage.use`
- `havennecessities.custommessage.custom`
- `havennecessities.professions.use`

## Admin

- `havennecessities.admin`
- `havennecessities.admin.balance`
- `havennecessities.admin.bank`
- `havennecessities.admin.serverbank`
- `havennecessities.admin.professions`
- `havennecessities.admin.storage`
- `havennecessities.admin.worldborder`
- `havennecessities.admin.reload`
- `havennecessities.admin.time`
- `havennecessities.admin.weather`
- `havennecessities.admin.broadcast`
- `havennecessities.admin.gamemode`
- `havennecessities.admin.heal`
- `havennecessities.admin.feed`
- `havennecessities.admin.fly`
- `havennecessities.admin.speed`
- `havennecessities.admin.item`
- `havennecessities.admin.item.unsafe`
- `havennecessities.admin.chat.discord`
- `havennecessities.admin.op`
- `havennecessities.admin.deop`
- `havennecessities.admin.clearchat`
- `havennecessities.admin.lockchat`
- `havennecessities.admin.kick`
- `havennecessities.admin.warn`
- `havennecessities.admin.ban`
- `havennecessities.admin.unban`
- `havennecessities.admin.mute`
- `havennecessities.admin.unmute`
- `havennecessities.admin.modhistory`

## Bypasses

- `havennecessities.flight.bypass-charge`
- `havennecessities.worldborder.bypass.teleport-block`
- `havennecessities.worldborder.bypass.placement-block`
- `havennecessities.inventory.bypass.death-rules`
- `havennecessities.professions.bypass.death-penalty`
- `havennecessities.combat.bypass.teleport-block`
- `havennecessities.combat.bypass.flight-block`
- `havennecessities.combat.bypass.logout-penalty`
